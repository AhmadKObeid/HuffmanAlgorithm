...
//adding controls to html page through Algorithm library
Huffman.prototype.addControls = function () {
    this.controls = [];
    //adding a text field to enter user input
  this.txtLabel = addLabelToAlgorithmBar("Text:");
  this.txtField = addControlToAlgorithmBar("Text", "");
  this.controls.push(this.txtField);
    //adding a build button to trigger the animation
  this.buildButton = addControlToAlgorithmBar("Button", "Build tree");
  this.buildButton.onclick = this.buildWrapper.bind(this);
  this.controls.push(this.buildButton);
    //adding a reset button to clear the canvas 
  this.resetButton = addControlToAlgorithmBar("Button", "Reset");
  this.resetButton.onclick = this.reset.bind(this);
  this.controls.push(this.resetButton);
};

//implemanting reset functionality
Huffman.prototype.reset = function () {
  ...
  this.cmd("CreateLabel",this.statusId,"Please Input a String and click 'Build Tree' to Start the animation!!",20,20,0);

  // taking the frequancany of each letter to reset the nodes the the initial position
  freqs = {};
  ...
  Object.keys(freqs)
   ...
   //adding the noodes created to the Priority 
  this.pq = new Huffman.PQ(this.roots,(node1, node2) => {
...
Huffman.prototype.disableUI = function () {
  this.setEnabled(false);
};

Huffman.prototype.enableUI = function () {
...
Huffman.prototype.buildWrapper = function (event) {
  ...
  this.implementAction(this.build.bind(this), this.txtField.value);
};
...
//=========== Starter function to begin the animation ================//
var currentAlg;
